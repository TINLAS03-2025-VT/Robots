...
float eval_poly3(float x, float coeff[4]) { return (coeff[0] * powf(x, 3)) + (coeff[1] * powf(x, 2)) + (coeff[2] * x) + coeff[3]; }

void encoder_callback(uint gpio, uint32_t events) {
	uint64_t current_tick = time_us_64();

	if (calibration_mode) {
		if (I_count == 0) cal_last_tick = current_tick;
		if (I_count == Target_I_count) cal_current_tick = current_tick;
		I_count++;
		return;
	}
	...
	if (delta < REFRACTORY_TIME_US) return; // Debounce filter
...
float measure_motor_speed(int pwm_val, char motor, int steps) {
	...
	while (I_count <= Target_I_count) sleep_ms(10);
	sleep_ms(500);
...
void calculate_PWM(float RPS_L, float RPS_R, int *pwm_L, int *pwm_R) {
	if (RPS_L > 0) *pwm_L = (int)(eval_poly3(RPS_L, coeff_LP) * 2000.0f + 5000.0f);
	else *pwm_L = (int)(eval_poly3(-RPS_L, coeff_LN) * 2000.0f + 5000.0f);

	if (RPS_R > 0) *pwm_R = (int)(eval_poly3(RPS_R, coeff_RP) * 2000.0f + 5000.0f);
	else *pwm_R = (int)(eval_poly3(-RPS_R, coeff_RN) * 2000.0f + 5000.0f);
}
