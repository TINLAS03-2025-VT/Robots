...
static float get_vector_length(const Vector3* vector) { return sqrtf(powf(vector->x, 2) + powf(vector->y, 2)); }
...
static void apply_goal_force(Vector3* total_force, const Vector3* current_pos, const Vector3* goal_pos) {
	...
	Vector3 unit_goal_2d_vector = { goal_vector.x / distance_to_goal, goal_vector.y / distance_to_goal, goal_vector.z };

	if (distance_to_goal < TARGET_SAFETY_MARGIN) {
		float force = (1.0f / distance_to_goal) * K_GOAL;
		unit_goal_2d_vector.x *= -force;
		unit_goal_2d_vector.y *= -force;
	}
	else {
		float force = distance_to_goal * K_GOAL;
		unit_goal_2d_vector.x *= force;
		unit_goal_2d_vector.y *= force;
	}

	total_force->x += unit_goal_2d_vector.x;
	total_force->y += unit_goal_2d_vector.y;
}
...
static float get_random_force() { return ((float)rand() / (float)RAND_MAX) * 2.0 - 1.0; }
...
void calculate_optimal_move(Vector3* optimal_move, const Vector3* start_pos, const Vector3Array* obstacle_poses,const Vector3* end_goal, const Vector3* excluded_point) {
	...
	if (force_mag < DISTANCE_INVERSION_SAFEGUARD) {
		force_mag = DISTANCE_INVERSION_SAFEGUARD;
	}
