...
int get_tag_pos(geometry_msgs__msg__Pose *robot_pose_buffer,
                geometry_msgs__msg__PoseArray *all_robot_positions,
                int tag_num) {
  for (int i = 0; i < (int)all_robot_positions->poses.size; i++) {
    if (all_robot_positions->poses.data[i].position.z == tag_num) {
      *robot_pose_buffer = all_robot_positions->poses.data[i];
      return 0;
    }
  }
  return 1; // Own robot isn't in array
}
...
void posearray_callback(const void *msgin) {
  ...
  //    else {
  //     printf("[WARNING] PoseArray received, but tag_num [%d] is not found in
  //     "
  //            "poses.size [%u]\n",
  //            tag_num, (unsigned int)all_robot_positions.poses.size);
  //   }
...
void setup_transport() {
  rmw_uros_set_custom_transport(
      false, &picow_params, picow_udp_transport_open, picow_udp_transport_close,
      picow_udp_transport_write, picow_udp_transport_read);
}
...
void ros_init() {
  ...
  // 	&publisher, &node,
  // 	ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt64), "pico_counter") !=
  // RCL_RET_OK ) HARDCHECK("Failed to initialize publisher");
...
int main() {
  ...
  set_pose(&target, 5.0, 5.0,
           0.0); // Set target Pose for testing point to point movement
  ...
  while (true) {
    srand(time(NULL)); // Produce seed for random vector in RFF algorithm
    ...
    if (current_time - last_pos_callback > MAX_MILLIS_WITHOUT_ANY_POSITION) {
      stop();
      HARDCHECK("Didn't receive any positions over topic: [%s]", POS_TOPIC);
    }

    if (current_time - last_own_pos_callback >
        MAX_MILLIS_WITHOUT_NEW_POSITION) {
      PRINT_DEBUG("No pos received for longer than %d seconds, stopping... \n",
                  MAX_MILLIS_WITHOUT_NEW_POSITION);
      stop();
    } else if (all_robot_positions.poses.size > 0 &&
               get_tag_pos(&own_robot_pose, &all_robot_positions, TAG_NUM) ==
                   0) {
      geometry_msgs__msg__Pose *next_step = geometry_msgs__msg__Pose__create();
      ...
      geometry_msgs__msg__Pose__destroy(next_step);
    }
  }
