...
void posearray_callback(const void *msgin) {
  ...
  if (all_robot_positions.poses.size > robot_num) {
    ...
    printf("[WARNING] PoseArray received, but robot_num [%d] is outside "
           "poses.size [%u]\n",
...
void ros_init() {
  ...
  if (rclc_publisher_init_best_effort(
          &publisher, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt64),
  ...
  if (rclc_timer_init_default2(&timer, &support, RCL_MS_TO_NS(1000),
                               timer_callback, true) != RCL_RET_OK) {
...
int main() {
  ...
  last_pos_callback = -1; // Set last_pos_callback to -1 to prevent driving
                          // without any received position
  while (true) {
    if (!wifi_connected()) {
      wifi_reconnect();
    }
