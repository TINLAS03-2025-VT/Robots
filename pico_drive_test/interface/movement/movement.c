...
// Calculate distance and angle from robot's Pose to target's Pose, turn or move forward based on DEADZONE
void move_to(geometry_msgs__msg__Pose *own_pos,
             geometry_msgs__msg__Pose *target_pos) {
  static bool stopped = false;

  if (calculate_distance(own_pos, target_pos) <= MIN_DISTANCE && !stopped){
    stopped = true;
    stop();
    return;
  }else{
...
// double map_range(double x, double in_min, double in_max, double out_min,
//            double out_max) {
